...
export default function Hero() {
  return (
    // REVISI 1: Ganti 'bg-slate-950' menjadi 'bg-transparent'
    // Ini kuncinya agar background Liquid Ether di layer belakang tidak tertutup
    <section className="relative min-h-screen flex items-center justify-center pt-20 overflow-hidden bg-transparent">
      {/* Efek Glow (Opsional - Bisa dihapus jika ingin Liquid Ether lebih jelas) */}
      {/* Saya biarkan opacity-nya rendah agar blending dengan background */}
      ...
      <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center relative z-10">
        ...
        <FadeIn>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <a href="#contact" className="px-8 py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition flex items-center justify-center gap-2 shadow-lg shadow-blue-500/25">
              Gas Mulai Projek <ArrowRight size={18} />
            </a>
            <a href="#projects" className="px-8 py-4 bg-white/5 hover:bg-white/10 text-white border border-white/10 font-bold rounded-xl transition text-center">
              Lihat Hasilnya Dulu
            </a>
          </div>
        </FadeIn>
        ...
        <FadeIn delay={0.2} className="relative h-[400px] lg:h-[600px] w-full flex items-center justify-center">
          {/* REVISI 2: Ubah background kartu jadi semi-transparan (Glassmorphism) */}
          <div className="relative w-full h-full bg-slate-900/40 backdrop-blur-md rounded-3xl border border-white/10 p-4 shadow-2xl flex items-center justify-center overflow-hidden group">
            {/* Noise texture overlay */}
            <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-20"></div>

            {/* Window Mockup */}
            <div className="absolute top-10 left-10 right-10 bottom-0 bg-slate-950/80 backdrop-blur-xl rounded-t-2xl border-t border-x border-white/10 p-6 shadow-2xl transition-transform duration-500 group-hover:-translate-y-4">
              {/* Tombol Mac style */}
